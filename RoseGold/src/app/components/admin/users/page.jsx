...
export default function page() {
  ...
  const handleSubmit = () => {
    ...
    const url = 'https://miivpersianshop.onrender.com/api/user';
    fetch(url, {
      method: 'POST',
...
function Row({ val, loadPage }) {
  ...
  const handleDelete = () => {
    ...
    if (window.confirm('Are you sure you want to delete this user?')) {
      fetch(`https://miivpersianshop.onrender.com/api/user/${val._id}`, {
        method: 'DELETE',
      })
        .then((res) => {
          if (res.ok) {
...
const loadPage = (setData) => {
  fetch('https://miivpersianshop.onrender.com/api/user')
    .then(res => {
      if (!res.ok) {

...
// export default function Cardd({ data }) {
    ...
    const router = useRouter();
      ...
      const handleDelete = async () => {
        ...
        if (confirm) {
          const res = await fetch(`https://miivpersianshop.onrender.com/api/products/${data._id}`, {
            method: 'DELETE',
          });
          if (res.ok) {
            onDelete(data._id);
            ...
            <Card sx={{ minWidth: 300, maxWidth: 350, border: '2px solid black' }}>
                <CardActionArea>
                    <div style={{ backgroundColor: '#faf7f1', mixBlendMode: 'multiply' }} className="firstt">
                        <CardMedia
                            component="img"
                            image={`https://miivpersianshop.onrender.com/${data.img}`}
                            alt={data.category}
                    ...
                    <CardActions sx={{ display: 'flex', justifyContent: 'flex-start' }}>
                        ...
                        {/* Example: Add to cart button using Redux */}
                        <Button
